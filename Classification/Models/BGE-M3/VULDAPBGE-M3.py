...
# import sys
# sys.path.append('../../vulDataClass.py')
import sys
    ...
    for infoData in informationData:
        ...
        for Threeshold in [0.60]:
            ...
            # #print(f"{count}TechnqiueID: {key}\tTechnique: {value['TechnqiueName']}\tDescription: {value['TechnqiueDescription']}")
                ...
                vul_data_array =[]
                external_embedding = model.encode(attack_texts)['dense_vecs']
                ...
                # count = count +1
        # df.to_excel(f"BGE-Final{infoData}_withOutPreProcessing2Full.xlsx", index=False)
        dfRes.to_excel(f"BGE-Final{infoData}_withOutPreProcessing22Full.xlsx", index=False)
