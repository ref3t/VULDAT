...
def checkCVEUsingAllTech():
    ...
    for mod in SentenceTransformersModels:
        ...
        trainDict = {}
        trainText = []
        for tech_dict in [train_data, test_data]:
            if tech_dict == validation_data:
                data = "Validation"
            elif tech_dict == test_data:
                data = "Test"
                print("test")
                for key, value in trainDict.items():                   
                    attack_texts = removeURLandCitationBulk([f"{value['TechnqiueName']} {value['TechnqiueDescription']}"])    
                    trainText.extend(attack_texts)
                trainEncode = model.encode(trainText)
            else:
                data = "Train"
                print("train")
                trainDict = tech_dict
            ...
            dataframeResults = pd.concat([dataframeResults, pd.DataFrame({'Model':[model_parts[1]+" "+data],'precision':[preci],'Recall':[Recal],'F1':[F1score]})], ignore_index=True)
            ...
            print(mod)
        dataframeResultsForallModels.to_excel(f"./results/AllModelsJaccardNew2.xlsx", index=False)
        dataframeResults.to_excel(f"./results/RResultsNew.xlsx", index=False)
        dfRes.to_excel(f"./results/{model_parts[1]}New.xlsx", index=False)
