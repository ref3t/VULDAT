...
def checkCVEUsingAllTech():
    ...
    for mod in SentenceTransformersModels:
        ...
        for infoData in informationData:
            ...
            print(mod)
            dataframeResultsForallModels.to_excel(f"VULDAT/Architectures/Results/AllModelsJaccardN{str(Threeshold)}2.xlsx", index=False)
            dataframeResults.to_excel(f"VULDAT/Architectures/Results/RResults502N{str(Threeshold)}2.xlsx", index=False)
            dfRes.to_excel(f"VULDAT/Architectures/Results/{model_parts[1]}1502NP{str(Threeshold)}2.xlsx", index=False)
